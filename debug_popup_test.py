...
def debug_popup_appearance():
    with sync_playwright() as p:
        ...
        try:
            ...
            test_file_path = r'f:\my_tools\MeeWoo\MeeWoo\test_files\test.svga'
            ...
            try:
                ...
                if dual_channel_button:
                    ...
                    if page.locator('.dual-channel-panel').count() > 0:
                        ...
                        # 检查组件的visible属性
                        print('检查组件的visible属性...')
                        try:
                            # 执行JavaScript获取组件的visible属性
                            visible_value = page.evaluate('''
                                () => {
                                    // 尝试找到Vue实例
                                    const app = document.querySelector('#app');
                                    if (app && app.__vue__) {
                                        // 找到dual-channel-panel组件
                                        const panel = app.__vue__.$children.find(child => child.$options.name === 'DualChannelPanel');
                                        if (panel) {
                            ...
                            ''')
                            print(f'dual-channel-panel组件的visible属性: {visible_value}')
                        except Exception as e:
                            print(f'无法获取组件的visible属性: {e}')
                            
                        # 检查activeRightPanel的值
                        print('检查activeRightPanel的值...')
                        try:
                            active_right_panel = page.evaluate('''
                                () => {
                                    const app = document.querySelector('#app');
                                    if (app && app.__vue__) {
                                        return app.__vue__.activeRightPanel;
                                    }
                                    return null;
                                }
                            ''')
                            print(f'activeRightPanel的值: {active_right_panel}')
                        except Exception as e:
                            print(f'无法获取activeRightPanel的值: {e}')
                    else:
        ...
        # 截图保存
        page.screenshot(path='f:\\my_tools\\MeeWoo\\MeeWoo\\debug_screenshot.png', full_page=True)

...
- **用户登录**：
  ...
  - 支持退出登录功能
- **用户类型控制**：
  - 基于登录状态自动判断用户层级：anonymous（未登录）→ loggedIn（登录普通用户）→ admin（登录管理员）
  - 层级继承规则：高级层级继承低级层级的配置，仅配置差异部分
  - 配置方式：通过 `site-config.json` 中的 `features.userLevels` 配置，支持 `hideButtons`、`showButtons` 和 `showAllButtons`
  - 控制逻辑：根据用户类型控制页面元素（如侧边栏按钮）的显示/隐藏
  - 实时检测：通过轮询机制实时检测登录状态变化，自动更新用户类型和页面显示
